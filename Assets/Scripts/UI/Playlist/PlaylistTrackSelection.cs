...
public class PlaylistTrackSelection : MonoBehaviour
{
    float defaultpos;
    ...
    public void OnPointerEnter(PointerEventData data)
    {
        ...
        if(!V2) GetComponent<RectTransform>().anchoredPosition = new Vector2(defaultpos - 20f, GetComponent<RectTransform>().anchoredPosition.y);
        ...
        if (!MainMenuManager.Singleton.noMoreSongsPlease)
            MainMenuManager.Singleton.LoadMusic(Path.Combine(Application.persistentDataPath, "songs", metadata.SongFileName), () =>
            {
                MainMenuManager.Singleton.menuMusic.Play(0, 50);
        ...
        GetComponent<RectTransform>().DOScale(1.065f, 0.1f).SetEase(Ease.OutBack);
    ...
    {
        GetComponent<RectTransform>().DOKill();
        GetComponent<RectTransform>().DOScale(1f, 0.15f).SetEase(Ease.OutExpo);
    }
